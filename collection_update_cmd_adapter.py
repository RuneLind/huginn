...
ap.add_argument("--contextual-model", required=False, default=None,
                help="Override contextual-prefix backend spec (e.g. 'ollama:qwen3.6:35b-a3b-nvfp4', 'claude-code:claude-haiku-4-5', 'anthropic:claude-haiku-4-5', 'none'). "
                     "If omitted, uses manifest['contextualPrefix']['model'] — letting scheduled updates inherit the create-time choice. Prefixes are retried once if the backend returns the wrong (non-zero) number of them; an empty result is a backend failure and is not retried.")
