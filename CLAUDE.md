...
| `nav-wiki` | `./<private-sub-repo>/wiki` | `index\.md` `log\.md` `CLAUDE\.md` `^\.obsidian/.*` `^\.claude/.*` |
| `wiki` | `./<private-sub-repo>/data/wiki` | `^index\.md$` `^log\.md$` `^CLAUDE\.md$` `^plans/.*` `^Clippings/.*` `^\..*` (dot-dirs: `.obsidian/`, `.smart-env/`, `.understand-anything/`) `^life/.*` `^trails\.json$` |
| `wiki-life` | `./<private-sub-repo>/data/wiki` | same, minus `^life/.*` — plus `includePatterns: ["^life/.*"]` |
